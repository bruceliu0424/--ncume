...
# 讀取照片並進行目標匹配
def detect_object_by_features(target_image_path, color_frame):
    ...
    if len(matches) > 10:
        ...
        # 繪製匹配到的特徵點
        matched_image = cv2.drawMatches(target_image, kp1, color_frame, kp2, matches[:20], None, flags=2)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            # 目標物品匹配與吸附
            # 目標物品匹配與吸附
            position, keypoints = detect_object_by_features(target_image_path, color_image)
            if position:
                x, y = position
                print(f"{target_name} 位置: {x}, {y}")

                # 在彩色影像上框選目標
                annotated_image = color_image.copy()  # 複製彩色影像避免影響原影像
                cv2.circle(annotated_image, (x, y), 10, (0, 255, 0), 2)  # 綠色圓圈
